        ...
        _data.read("users", tokenData.hashedEmail, function(err, userData) {
          if (!err && userData) {
            ...
            if (cartData && cartItems && cartAmount) {
              ...
              cartData.createdAt = Date.now()

              const payload = {
                amount: cartData.amount,
                currency: "usd",
                description: "charge for order id: " + cartData.id,
                source: paymentToken,
              };
              ...
              const request = https.request(requestDetails, function(response) {
                ...
                if (status === 200) {
                  const itemsDetails = cartData.items.map(function(item, idx) {
                    return `${idx +
                      1}. ${item.displayName} || Quantity: ${item.qty} || Amount: ${((item.price * item.qty)/100).toFixed(2)}\n`;
                  });
                  const msg = `
  Your order number ${cartData.id} for USD ${(cartData.amount /
                    100).toFixed(2)} has been paid.

  Details:
  ${itemsDetails.join("")}

  Total Amount Paid: ${(cartData.amount/100).toFixed(2)}
                        ...
                        function(err) {
                          if (!err) {
                            helpers.mailGunJob(
                              ...
                              function(err) {
                                if (!err) {
                                  cartData.mailSent = true;
                                  _data.update("orders",orderId,cartData,function (err) {
                                    if(!err){
                                      cb(200);
                                    } else {
                                      cb(500, {
                                        Error: "Could not update order data",
                                      });
                                    }
                                  })
                                } else {
                                  console.log(err);
                                  cb(500, {
                                    Error: "Could not email the user",
                                  });
                                }
                              }
                            );
